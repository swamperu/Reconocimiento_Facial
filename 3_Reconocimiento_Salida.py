...
with conectar_bd() as cnxn:
    with cnxn.cursor() as cursor:
        ...
        camara_id = int(parametros.get("CAMARA_SALIDA", '1'))
        cap = cv2.VideoCapture(camara_id, cv2.CAP_DSHOW)
        cv2.namedWindow("Reconocimiento Facial Salida", cv2.WINDOW_NORMAL)  # Permite maximizar la ventana
        if not cap.isOpened():
        ...
        try:
            while True:
                ...
                if registro_exitoso and tiempo_registro is not None and (datetime.now() - tiempo_registro).total_seconds() < int(tiempo_mensaje_confirmacion):
                    ...
                    cv2.imshow("Reconocimiento Facial Salida", frame)
                ...
                cv2.imshow("Reconocimiento Facial Salida", frame)
                cv2.namedWindow("Reconocimiento Facial Salida", cv2.WINDOW_NORMAL)
